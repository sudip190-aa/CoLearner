...
const project = (n) => (n.target?.slug ? `/projects/${n.target.slug}` : '/projects')
const workspace = (n) =>
  n.target?.slug ? `/projects/${n.target.slug}/workspace` : '/projects'
const thread = (n) => (n.target?.slug ? `/community/${n.target.slug}` : '/community')
...
const VERBS = {
  ...
  connection_accepted: { phrase: 'accepted your connection request', to: person },
  project_join_request: { phrase: 'asked to join', to: workspace },
  project_join_approved: { phrase: 'accepted your request to join', to: project },
  project_join_rejected: { phrase: 'declined your request to join', to: project },
  project_invite: { phrase: 'invited you to join', to: project },
  project_invite_accepted: { phrase: 'accepted your invitation to', to: workspace },
  project_invite_declined: { phrase: 'declined your invitation to', to: project },
...
export function describeNotification(notification) {
  ...
  if (verb === 'level_up') {
    return { actorName: '', phrase: 'You reached a new level. Keep going!', label: '', to: '/leaderboard' }
  }
  if (verb.startsWith('badge_')) {
    return {
      actorName: '',
      phrase: 'You earned the',
      label: notification.target?.label ? `${notification.target.label} badge` : 'a new badge',
      to: '/badges',
    }
  }
