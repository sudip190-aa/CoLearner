-- Preserve the twelve-week heatmap and profile statistic contracts.
create or replace function public.colearn_portfolio(username text) returns jsonb language plpgsql stable security definer set search_path='' as $$ declare p public.profiles;begin
 select * into p from public.profiles where lower(profiles.username)=lower(colearn_portfolio.username) and is_active;
