-- Preserve deterministic order through JSON aggregation, including tied timestamps and pagination.
create or replace function public.colearn_project_feed(filters jsonb default '{}') returns jsonb language plpgsql stable security invoker set search_path='' as $$
...
begin
 ...
 with visible as (
  ...
  select p.*,row_number() over(order by
   ...
   case when filters->>'ordering'='updated' then updated_at end desc nulls last,created_at desc,id desc) as feed_position
  from filtered p order by
   case when filters->>'ordering'='fewest' then spots_left end asc nulls last,
 ...
 ) select jsonb_build_object('count',(select count(*) from filtered),'results',coalesce((select jsonb_agg((to_jsonb(s)-'feed_position')||jsonb_build_object('task_progress',jsonb_build_object('total',coalesce(c.total,0),'done',coalesce(c.done,0),'percent',coalesce(c.percent,0))) order by s.feed_position) from selected s left join public.colearn_project_stats() c on c.project_id=s.id),'[]'),
...
-- Existing RLS protects live catalog and membership changes.
do $$ begin
 if not exists(select 1 from pg_publication_tables where pubname='supabase_realtime' and tablename='projects') then alter publication supabase_realtime add table public.projects;end if;
 if not exists(select 1 from pg_publication_tables where pubname='supabase_realtime' and tablename='project_members') then alter publication supabase_realtime add table public.project_members;end if;
end $$;
