...
// --use-file-for-fake-audio-capture=<absolute .dist/voice-test.wav>
// Generate it with: node supabase/scripts/create-test-audio.mjs
...
const instrument = `(() => {
  ...
  window.RTCPeerConnection=class extends Original { constructor(...args){super(...args);window.voiceTest.pcs.push(this)} addTrack(track){return super.addTrack(track)} };
...
try {
  ...
  let denied = await alice.rpc("colearn_start_call", {
    peer: c.id,
    device_id: randomUUID(),
  });
  assert.equal(denied.error?.code, "42501");
  denied = await client().rpc("colearn_start_call", {
    peer: b.id,
    device_id: randomUUID(),
  });
  assert(denied.error);
  ...
  const race = await Promise.all([
    alice.rpc("colearn_start_call", { peer: b.id, device_id: ad }),
    bob.rpc("colearn_start_call", { peer: a.id, device_id: bd }),
  ]);
  assert.equal(race.filter((r) => !r.error).length, 1);
  const winner = race[0].error ? 1 : 0,
    record = race[winner].data,
    owner = clients[winner],
    ownerDevice = winner ? bd : ad;
  assert.equal((await ok(outsider.from("voice_calls").select("*"))).length, 0);
  assert(
    (
      await outsider.rpc("colearn_call_action", {
        call_id: record.id,
        operation: "end",
        device_id: randomUUID(),
      })
  ...
  assert(
    (
      await owner.rpc("colearn_call_action", {
        call_id: record.id,
        operation: "heartbeat",
        device_id: randomUUID(),
      })
  ...
  pass(
    "Simultaneous cross-calls serialized; RLS, direct writes, device ownership protected",
  );
  // Real Realtime join authorization, including a peer trying to forge the other sender.
  async function join(c, topic) {
  ...
  const rejected = await join(
    outsider,
    `voice:${record.id}:${record.caller_id}`,
  );
  assert.notEqual(rejected.status, "SUBSCRIBED");
  const forged = await join(owner, `voice:${record.id}:${record.receiver_id}`);
  assert.equal(forged.status, "SUBSCRIBED");
  assert.notEqual(
    await forged.channel.send({
      type: "broadcast",
      event: "signal",
      payload: { kind: "end" },
    }),
  ...
  const receiver = clients[winner ? 0 : 1],
    receiverDevice = randomUUID();
  await ok(
    receiver.rpc("colearn_call_action", {
      call_id: record.id,
      operation: "accept",
      device_id: receiverDevice,
    }),
  );
  assert(
    (
      await receiver.rpc("colearn_call_action", {
        call_id: record.id,
        operation: "accept",
        device_id: randomUUID(),
      })
  ...
  assert(
    (
      await owner.rpc("colearn_start_call", {
        peer: record.receiver_id,
        device_id: randomUUID(),
      })
    ).error,
  );
  pass(
    "Accepted call belongs to one receiving device; additional calls rejected",
  );
  await ok(
    owner.rpc("colearn_call_action", {
      call_id: record.id,
      operation: "end",
  ...
  await ok(
    admin
      ...
      .update({
        ...
        caller_seen_at: new Date(Date.now() - 60000).toISOString(),
      })
      .eq("id", expired.id),
  );
  ...
  assert(
    (await alice.rpc("colearn_start_call", { peer: b.id, device_id: ad }))
      .error,
  );
  await ok(
    admin
  ...
  await ok(
    admin.from("voice_calls").delete().in("id", [expired.id, replacement.id]),
  );
  pass(
    "Expired leases release busy users; revoked connections end calls and reject new calls",
  );
  for (let i = 0; i < 3; i++) {
    const old = await ok(
      alice.rpc("colearn_start_call", { peer: b.id, device_id: ad }),
    ...
    await ok(
      admin
        .from("voice_calls")
        .update({ caller_seen_at: new Date(Date.now() - 44900).toISOString() })
        .eq("id", old.id),
    );
    await Promise.all([
      alice.rpc("colearn_call_action", {
        call_id: old.id,
        operation: "heartbeat",
        device_id: ad,
      }),
    ...
    const records = await ok(
      admin
        ...
        .select("*")
        .eq("caller_id", a.id)
        .or("status.eq.ringing,status.eq.accepted"),
    );
    ...
    assert(
      [...records, ...other].filter(
        (r) =>
          Date.now() - Date.parse(r.caller_seen_at) < 45000 &&
          Date.now() - Date.parse(r.created_at) < 45000,
      ).length <= 1,
    );
    await ok(admin.from("voice_calls").delete().in("caller_id", [a.id, b.id]));
  }
  pass(
    "Concurrent heartbeat and replacement-call requests leave at most one live call",
  );
  ...
  for (const p of [A, B]) {
    const stats = await p.evaluate(
      "voiceTest.pcs.at(-1).getStats().then(s=>Array.from(s.values()).filter(r=>r.type==='inbound-rtp'&&r.kind==='audio').map(r=>({packets:r.packetsReceived,energy:r.totalAudioEnergy})))",
    );
    assert(
      stats.some((s) => s.packets > 0 && s.energy > 0),
      "No received audio energy",
    );
    assert(
      await p.evaluate(
        "[...document.querySelectorAll('audio')].some(a=>a.srcObject && !a.paused && !a.muted && a.volume>0)",
      ),
      "Remote output is not playing",
    );
    await p.wait(
      "document.querySelector('meter[aria-label=\"Incoming audio level\"]')?.value > 0.001",
    );
  }
  pass(
    "Connected invitation + accept; bidirectional WebRTC audio packets and nonzero audio energy",
  );

  // Exercise the real renegotiation path after a simulated disconnect event.
  // The transport remains local; this is not a physical network-handover test.
  const beforeIce = await A.evaluate(
    "voiceTest.pcs.at(-1).localDescription.sdp.split('a=ice-ufrag:')[1].split(String.fromCharCode(13))[0]",
  );
  await A.evaluate(
    "(()=>{const pc=voiceTest.pcs.at(-1);Object.defineProperty(pc,'connectionState',{configurable:true,value:'disconnected'});pc.onconnectionstatechange();delete pc.connectionState})()",
  );
  await A.wait(
    `voiceTest.pcs.at(-1).localDescription.sdp.split('a=ice-ufrag:')[1].split(String.fromCharCode(13))[0]!==${JSON.stringify(beforeIce)} && voiceTest.pcs.at(-1).signalingState==='stable'`,
  );
  await Promise.all([A.connected(), B.connected()]);
  for (const P of [A, B])
    await P.wait("document.querySelector('meter')?.value>0.001");
  pass(
    "ICE restart renegotiates credentials and restores connected UI with two-way decoded audio",
  );
  ...
  await A.click("Mute");
  assert(await A.evaluate("voiceTest.tracks.every(t=>!t.enabled)"));
  await A.click("Unmute");
  assert(await A.evaluate("voiceTest.tracks.every(t=>t.enabled)"));
  await A.click("End call");
  await Promise.all([A.clean(), B.clean()]);
  pass(
    "Mute/unmute and caller hang-up release tracks, peers and call channels",
  );

  const declineNetwork = badNetwork.length;
  await A.call();
  await B.click("Decline");
  await Promise.all([A.clean(), B.clean()]);
  await A.wait("document.body.innerText.includes('Call declined')");
  await pause(1500);
  const declinedRequests = badNetwork.splice(declineNetwork);
  assert(
    declinedRequests.every(
      (r) => r.status === 403 && r.path === "/functions/v1/voice-ice",
    ),
  );
  expectedNetwork.push(...declinedRequests);
  ...
  const original = await A.evaluate(
    "(()=>{window.originalVoiceMic=navigator.mediaDevices.getUserMedia;navigator.mediaDevices.getUserMedia=async()=>{throw new DOMException('Denied','NotAllowedError')};return true})()",
  );
  assert(original);
  await A.call();
  await A.wait(
  ...
  await A.clean();
  await A.evaluate(
    "navigator.mediaDevices.getUserMedia=window.originalVoiceMic",
  );
  pass("Microphone denial produces actionable UI without opening a call");
  await A.evaluate(
    "navigator.mediaDevices.getUserMedia=async()=>{throw new DOMException('Missing','NotFoundError')}",
  );
  await A.call();
  await A.wait("document.body.innerText.includes('No microphone found')");
  await A.clean();
  await A.evaluate(
    "navigator.mediaDevices.getUserMedia=window.originalVoiceMic",
  );
  await A.evaluate(
    "(()=>{window.originalVoiceRTC=window.RTCPeerConnection;window.RTCPeerConnection=undefined})()",
  );
  await A.call();
  await A.wait("document.body.innerText.includes('supported browser')");
  await A.clean();
  ...
  await A.evaluate(
    "(()=>{const el=document.querySelector('textarea[aria-label=Message]');Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype,'value').set.call(el,'Voice regression: text still works');el.dispatchEvent(new Event('input',{bubbles:true}))})()",
  );
  await A.evaluate(
    "document.querySelector('button[aria-label=\"Send message\"]').click()",
  );
  await B.wait(
    "document.querySelector('[role=log]')?.innerText.includes('Voice regression: text still works')",
  );
  ...
  await B2.call();
  await A.click("Accept");
  await Promise.all([A.connected(), B2.connected()]);
  await B.wait(
    "document.querySelector('button[aria-label^=\"Voice call\"]') && !document.querySelector('[role=dialog]')",
  );
  const networkBefore = badNetwork.length,
    errorsBefore = errors.length;
  await B.call();
  await B.wait("document.body.innerText.includes('already in a call')");
  await B.clean();
  const rejectedCalls = badNetwork.splice(networkBefore);
  assert(
    rejectedCalls.length &&
      rejectedCalls.every(
        (r) => r.status === 400 && r.path === "/rest/v1/rpc/colearn_start_call",
      ),
  );
  ...
  await B2.cmd("Page.navigate", { url: "about:blank" });
  await A.clean();
  pass(
    "Other tabs cannot start overlapping calls; leaving the page ends the remote call",
  );
  // A pending permission prompt can be canceled, including a late permission grant.
  await A.evaluate(
    "(()=>{window.originalVoiceMic=navigator.mediaDevices.getUserMedia;navigator.mediaDevices.getUserMedia=()=>new Promise(resolve=>{window.resolveLateMic=resolve})})()",
  );
  await A.call();
  await A.click("End call");
  await A.evaluate(
    "originalVoiceMic({audio:true}).then(stream=>window.resolveLateMic(stream))",
  );
  await A.clean();
  await A.evaluate(
    "navigator.mediaDevices.getUserMedia=window.originalVoiceMic",
